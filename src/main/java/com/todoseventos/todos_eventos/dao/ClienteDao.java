...
public class ClienteDao {
    ...
    public ClienteModel procurarPorCpf(String cpf) {
        try {
            String sql = "SELECT p.*, pf.cpf, pf.dataNascimento FROM PESSOA p LEFT JOIN PESSOA_FISICA pf ON p.id_pessoa = pf.id_pessoa " +
                    "WHERE pf.cpf = ?";
    ...
    public ClienteModel procurarPorCnpj(String cnpj) {
        try {
            String sql = "SELECT p.*, pj.cnpj FROM PESSOA p LEFT JOIN PESSOA_JURIDICA pj ON p.id_pessoa = pj.id_pessoa " +
                    "WHERE pj.cnpj = ?";
